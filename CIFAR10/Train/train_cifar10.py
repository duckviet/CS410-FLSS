...
def train(args, model, device, train_loader, optimizer, epoch,scheduler,awp_adversary):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss_soft_adv1 = 0.1 * torch.nn.functional.kl_div(pred_soft_sample, pred_soft_no_sample, reduction='batchmean')
        ...
        loss_KLD2_clean = torch.nn.functional.kl_div(pred_soft_sample, pred_soft_no_sample, reduction='batchmean')
...
def get_latest_checkpoint():
    checkpoint_files = glob.glob(os.path.join(model_dir, 'model-res18FC-epoch*.pt'))
    if not checkpoint_files:
        return None, None, 0
    latest_model = max(checkpoint_files, key=os.path.getctime)
    latest_opt = latest_model.replace('model-res18FC-epoch', 'opt-res18FC-checkpoint_epoch').replace('.pt', '.tar')
    current_epoch = int(latest_model.split('epoch')[-1].split('.')[0])
    return latest_model, latest_opt, current_epoch

def get_latest_log_epoch(log_file):
    ...
    with open(log_file, 'r') as f:
        ...
        if not lines:
            return 0
        # Get the last line that contains epoch information
...
def main():
    ...
    latest_model, latest_opt, current_epoch = get_latest_checkpoint()
    start_epoch = 1
    
    # Get the latest epoch from log files
    latest_log_epoch = max(
        get_latest_log_epoch(LOSS_TRAIN),
        get_latest_log_epoch(ACC_PGD10_TRAIN),
        get_latest_log_epoch(ACC_CLEAN_TRAIN)
    )
    
    if latest_model and latest_opt:
        print(f"Loading checkpoint: {latest_model}")
        model.load_state_dict(torch.load(latest_model))
        optimizer.load_state_dict(torch.load(latest_opt))
        start_epoch = max(current_epoch + 1, latest_log_epoch + 1)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Save model and optimizer with current epoch number
